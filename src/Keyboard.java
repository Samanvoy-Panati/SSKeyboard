...
public class Keyboard{
	...
	public Keyboard(Dictionary dict) throws FileNotFoundException{
		...
		input = new JTextField("ALWAYS WRITE DOWN WHO IS RESPONSIBLE FOR WHAT BY WHEN");
		...
		Color clr = new Color(200,100,100);
		BACK.setBackground(clr);
		BACK.setFont(new Font("ALGERIAN",Font.BOLD, 16));
		//BACK.setForeground(clr);
		addKey(board,BACK,10+1,1,2,1);
		...
		CLEAR_WORD.setFont(new Font("ALGERIAN",Font.BOLD, 16));
		//CLEAR_WORD.setForeground(clr);
		...
		clr = new Color(242,242,28);
		CLEAR_TEXT.setBackground(clr);
		CLEAR_TEXT.setFont(new Font("ALGERIAN",Font.BOLD, 16));
		//CLEAR_TEXT.setForeground(clr);
		
		addKey(board,CLEAR_TEXT,7+2,3,4,1);		
		...
		final JButton STATS = new JButton("LEVENSHTEIN DISTANCE");
		clr = new Color(100,100,200);
		STATS.setBackground(clr);
		STATS.setFont(new Font("ALGERIAN",Font.BOLD, 16));
		//STATS.setForeground(clr);
		addKey(board,STATS,9,4,4,1);
		...
		STATS.addMouseListener(new MouseListener() {
			...
			public int levenshteinDistance(String output){
				...
				System.out.println("OUTPUT: "+output.substring(0, output.length()-2)+"  length:"+y);
				System.out.println("GIVEN_SENTENCE: "+given_input+"  length"+x);
				//If there is only '_'
				if(output.length()==1){
			...
			public void traceback(int[][] matrix, int x, int y, String given_input, String output){
				...
				steps = steps+"Levenshtein Distance: \t"+matrix[x][y]+"\n";
				...
				//for insertions
				int e=0;
				for(int a=0;a<deletion.length;a++){
					insertion[a] = deletion[a];
				}
				for(int a=deletion.length;a<(y-deletions+insertions);a++){
					insertion[a] = "";
				}
				System.out.println("deletion.length: "+deletion.length);
				...
				for(int b=0;b<(y-deletions+insertions);b++){
					...
					if(oper_input[b]==2){
						for(e=(y-deletions+insertions-2);e>=b;e--){
	...
	public ArrayList<String> predictWord(String trace_word,ArrayList<String> sure_characters){
		...
		for(int i=0;i<sure_characters.size();i++){
			for(int j=0;j<unsorted_words.size();j++){
				//if the sure character is not present in the predicted word then store the index and later remove it
				if( !(unsorted_words.get(j).contains(sure_characters.get(i))) ){
					if( !(remove_index.contains(unsorted_words.get(j))) ){
						remove_index.add(unsorted_words.get(j));
					}
				}
			}
		}
		...
		for(int i=0;i<remove_index.size();i++){
			unsorted_words.remove(remove_index.get(i));
			dup_unsorted_words.remove(remove_index.get(i));
		}
	...
	class MouseListener extends MouseAdapter implements MouseMotionListener{
		...
		ArrayList<String> sure_characters;
		Key curKey;
		int isPressed = 0;
		...
		MouseListener() throws FileNotFoundException{
			...
			sure_characters = new ArrayList<String>();
			curKey = new Key("");
		}
		...
		public void normalState(ArrayList<Key> tracelist){
			...
			System.out.println("Number of Sure characters: "+sure_characters.size());
			System.out.println("Sure Characters are: ");
			for(int i=0;i<sure_characters.size();i++){
				System.out.print(sure_characters.get(i)+"  ");
			}
			sure_characters.clear();
		}
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (SwingUtilities.isRightMouseButton (e))
	        {
				...
				sure_characters.add(theEventer.getText());
	        }
		...
		public void mouseReleased(MouseEvent e) {
			...
			if(SwingUtilities.isRightMouseButton (e)){
				...
				if(!tracelist.isEmpty()){
					sure_characters.add(tracelist.get(tracelist.size()-1).getText());
				}
			}
			else
			{
				...
				else
				{
					...
					ArrayList<String> predicted_words = predictWord(trace_word, sure_characters);
					String output = "";
	...
	public static void main(String[] args) throws FileNotFoundException,IOException {
		System.out.println("SS KEYBOARD");
